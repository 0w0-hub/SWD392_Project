...
> Bài này trình bày **Delivery Order Service** đúng theo cách sách thiết kế nó, rồi ánh xạ sang bản cài đặt của em (**NestJS + MongoDB / Mongoose, TypeScript**).
...
5. Em cài đặt đúng interface đó bằng NestJS controller (10 endpoint) + Mongoose model đúng schema thực thể **DeliveryOrder / Item / Invoice**.
...
**Ánh xạ sang code Mongoose** (`schemas/`):

- `DeliveryOrder` → `schemas/delivery-order.schema.ts`: đủ 11 field trên; `orderId` đánh dấu `unique`, `orderStatus` dùng `enum: OrderStatus, default: NotYetShipped`, `items` là mảng sub-document nhúng `[ItemSchema]` → đúng nghĩa **aggregation** trong sách (Item nhúng trong Order, `_id: false`).
...
pnpm install
pnpm run start:dev     # http://localhost:3001, Mongo: mongodb://localhost:27017/delivery-order
```
...
| Sao dùng NestJS + MongoDB mà khác Inventory (Express + SQLite)? | SOA cho phép mỗi service tự chọn stack, miễn tuân đúng interface đã đăng ký với broker (§22.6, §22.7.6 — binding động). Đây là tính **độc lập** của service. |
...
- **Schema DeliveryOrder + index** → `src/delivery-order/schemas/delivery-order.schema.ts` (aggregation `items` dòng 45, index dòng 51)
...
- **Đăng ký model + DI** → `src/delivery-order/delivery-order.module.ts`; kết nối Mongo → `src/app.module.ts`
