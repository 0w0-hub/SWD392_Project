  ...
  implements OnApplicationBootstrap, OnApplicationShutdown
{
  ...
  private async register(): Promise<void> {
    try {
      ...
      this.logger.warn(
        `Broker is unavailable; Delivery Order API remains online and registration will retry: ${this.message(error)}`,
      );
    }
  }
  ...
  private createRegistration(): ServiceRegistration {
    const port = this.readPositiveInteger(process.env.PORT, 3001);
    ...
    return {
      serviceId:
        process.env.SERVICE_ID ?? `delivery-order-service-${process.pid}`,
