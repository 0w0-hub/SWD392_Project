...
describe('BrokerRegistrationService', () => {
  ...
  beforeEach(() => {
    ...
    process.env.SERVICE_ID = 'catalog-test-1';
    process.env.SERVICE_HOST = 'catalog.test';
    process.env.SERVICE_BASE_URL = 'http://catalog.test:3000/';
  ...
  it('registers the Catalog operations and deregisters on shutdown', async () => {
    ...
    expect(fetchSpy).toHaveBeenNthCalledWith(
      ...
      expect.objectContaining({
        method: 'POST',
        body: JSON.stringify({
          serviceId: 'catalog-test-1',
          serviceName: 'CatalogService',
          version: '0.0.1-test',
          host: 'catalog.test',
          port: 3000,
          baseUrl: 'http://catalog.test:3000',
          healthUrl: 'http://catalog.test:3000/health',
    ...
    expect(fetchSpy).toHaveBeenNthCalledWith(
      2,
      'http://broker.test:8080/registry/services/catalog-test-1',
  ...
  it('re-registers when a heartbeat reports that the Broker forgot it', async () => {
    ...
    expect(fetchSpy.mock.calls.map(([url]) => url)).toEqual([
      'http://broker.test:8080/registry/services',
      'http://broker.test:8080/registry/services/catalog-test-1/heartbeat',
      'http://broker.test:8080/registry/services',
    ]);
